"""Controller HTTP de clientes, historico e pipeline CRM."""
...
from app.controllers.http import company_local, failure, model_dict, payload, record_audit, success
from app.services.customer_service import CRM_STAGES
...
def index():
    ...
    query = ListCustomersService().execute(
        company_id, search=search, stage=stage, status=status
    )
...
def create():
    ...
    if data.get("customer_id"):
        try:
            ...
            customer = UpdateCustomerService().execute(
                current_company_id(), customer_id, commit=False, **changes
            )
...
def update(customer_id: int):
    ...
    try:
        customer = UpdateCustomerService().execute(
            current_company_id(), customer_id, commit=False, **allowed
        )
    except DomainError as exc:
        return failure(exc.message, errors=exc.details, status=exc.status_code)
...
def archive(customer_id: int):
    customer = ArchiveCustomerService().execute(
        current_company_id(), customer_id, commit=False
    )
...
def crm():
    company_id = current_company_id()
    customers = ListCustomersService().execute(company_id).limit(500).all()
...
def update_stage(customer_id: int):
    ...
    try:
        customer = UpdateCustomerService().execute(
            current_company_id(), customer_id, crm_stage=stage, commit=False
        )
    except DomainError as exc:
...
def add_tag(customer_id: int):
    data = payload()
    try:
        tag = AddCustomerTagService().execute(
            current_company_id(),
            customer_id,
...
def save_note(customer_id: int):
    ...
    try:
        customer = UpdateCustomerService().execute(
            current_company_id(), customer_id, notes=note, commit=False
        )
    except DomainError as exc:
        return failure(exc.message, status=exc.status_code)
...
class CustomersController:
    """Handlers HTTP do dominio de clientes."""
    ...
    crm = staticmethod(crm)
    update_stage = staticmethod(update_stage)
    add_tag = staticmethod(add_tag)
    remove_tag = staticmethod(remove_tag)
    save_note = staticmethod(save_note)


for _endpoint, _handler in {
    "index": CustomersController.index,
    "create": CustomersController.create,
    "detail": CustomersController.detail,
    "update": CustomersController.update,
    "archive": CustomersController.archive,
    "crm": CustomersController.crm,
    "update_stage": CustomersController.update_stage,
    "add_tag": CustomersController.add_tag,
    "remove_tag": CustomersController.remove_tag,
    "save_note": CustomersController.save_note,
}.items():
