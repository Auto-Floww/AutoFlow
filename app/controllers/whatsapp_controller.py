...
from app.services.outbox import DispatchOutboxBestEffortService
from app.services.whatsapp import ProcessWhatsAppPayloadService
...
def evolution_webhook():
    ...
    try:
        events = ProcessWhatsAppPayloadService(service).execute(
            data, allowed_instances={instance_name}
        )
    ...
    for event in events:
        if event.get("kind") != "message" or not event.get("outbox_id"):
            continue
        if DispatchOutboxBestEffortService().execute(event["outbox_id"]):
            queued += 1
        else:
            outbox_pending += 1
    return jsonify(
...
bp.view_functions["evolution_webhook"] = WhatsAppController.evolution_webhook
