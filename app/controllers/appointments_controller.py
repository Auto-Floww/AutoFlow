...
def index():
    ...
    local_now = AppointmentOperations.to_local(company_id, utcnow())
    ...
    start = AppointmentOperations.normalize_datetime(company_id, local_start)
    local_end = local_start + timedelta(days=7)
    end = AppointmentOperations.normalize_datetime(company_id, local_end)
    appointments = (
    ...
    for item in appointments:
        local_item_start = AppointmentOperations.to_local(company_id, item.starts_at)
        local_item_end = AppointmentOperations.to_local(company_id, item.ends_at)
    ...
    today = AppointmentOperations.normalize_datetime(company_id, local_today)
    tomorrow = AppointmentOperations.normalize_datetime(
    ...
    block_views = [
        SimpleNamespace(
            ...
            title=block.title,
            period_label=(
                f"{AppointmentOperations.to_local(company_id, block.starts_at).strftime('%d/%m/%Y %H:%M')} a "
                f"{AppointmentOperations.to_local(company_id, block.ends_at).strftime('%d/%m/%Y %H:%M')}"
            ),
        )
        for block in blocks
    ]
...
def availability():
    try:
        slots = GetAvailableAppointmentsService().execute(
            current_company_id(),
...
def cancel(appointment_id: int):
    try:
        appointment = CancelAppointmentService().execute(
...
def create_block():
    ...
    try:
        company_id = current_company_id()
        start = AppointmentOperations.normalize_datetime(company_id, data.get("starts_at", ""))
        end = AppointmentOperations.normalize_datetime(company_id, data.get("ends_at", ""))
        if end <= start:
            raise ValueError
        professional_id = int(data["professional_id"]) if data.get("professional_id") else None
        if professional_id:
...
class AppointmentsController:
    ...
    create_professional = staticmethod(create_professional)
    save_business_hour = staticmethod(save_business_hour)
    create_block = staticmethod(create_block)
    delete_block = staticmethod(delete_block)
