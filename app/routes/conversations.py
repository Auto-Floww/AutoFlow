...
from app.routes.helpers import company_local, failure, model_dict, payload, record_audit, success, wants_json
...
def create():
    ...
    company_id = current_company_id()
    try:
        customer = CustomerService.find_by_phone(company_id, phone)
        if customer is None:
            customer = CustomerService.create(
                company_id, name=str(data.get("name", phone)), phone=phone, source="MANUAL"
            )
        conversation, _ = ConversationService.get_or_create(
            company_id, customer_id=customer.id, channel="WHATSAPP"
        )
        ConversationService.claim(company_id, conversation.id, user_id=current_user.id)
...
def return_to_ai(conversation_id: int):
    try:
        conversation = ConversationService.return_to_ai(
            current_company_id(), conversation_id, user_id=current_user.id
        )
    except DomainError as exc:
        return failure(exc.message, status=exc.status_code)
...
def add_tag(conversation_id: int):
    ...
    if not exists:
        db.session.add(
            ConversationTag(company_id=company_id, conversation_id=conversation.id, tag_id=tag.id)
        )
        db.session.commit()
