"""Saldos e razao imutavel de estoque."""
...
from app.routes.helpers import failure, model_dict, payload, record_audit, success
...
def index():
    ...
    low_stock_count = len(InventoryService.low_stock(company_id))
    inventory_views = []
    total_units = 0
...
def movement(inventory_id: int | None = None):
    ...
    try:
        if data.get("target_quantity") not in {None, ""} or movement_type == "ADJUSTMENT":
            ...
            result = InventoryService.set_stock(
                current_company_id(),
                inventory_id,
                quantity=int(target),
                reason=reason,
...
def history(inventory_id: int):
    inventory = tenant_get_or_404(Inventory, inventory_id)
    movements = inventory.movements.order_by(InventoryMovement.created_at.desc()).limit(250).all()
    return {
