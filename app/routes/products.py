"""Catalogo de produtos e variantes."""
...
def index():
    ...
    if search:
        pattern = f"%{search}%"
        query = query.filter(
            or_(Product.name.ilike(pattern), Product.sku.ilike(pattern), Product.brand.ilike(pattern))
        )
    if category:
...
def create():
    ...
    try:
        ...
        has_variants = any(
            any(str(value).strip() for value in values) for values in (colors, sizes, skus)
        )
        if has_variants:
            for index in range(variant_count):
                ...
                variant = ProductVariant(
                    ...
                    color=color[:80] or None,
                    size=size[:80] or None,
                    sku=sku[:80] or None,
                    price=_money(price, optional=True),
                    is_active=True,
                )
                db.session.add(variant)
                db.session.flush()
                variant_inventory = InventoryService.get_or_create(
                    product.company_id,
            ...
            inventory = InventoryService.get_or_create(
                product.company_id,
                product_id=product.id,
                minimum_quantity=max(0, int(data.get("minimum_quantity", 0) or 0)),
                commit=False,
...
def update(product_id: int):
    ...
    try:
        ...
        if pending_image:
            product.image_url = pending_image.public_url
        for field, maximum in {
            ...
            if field in data:
                ...
                setattr(product, field, value[:maximum] or None)
        if not product.name:
            raise ValueError("Informe o nome do produto")
        if "price" in data:
            product.price = _money(data["price"])
        if "promotional_price" in data:
            product.promotional_price = _money(data["promotional_price"], optional=True)
        if "active" in data or "is_active" in data:
            product.is_active = coerce_bool(data.get("active", data.get("is_active")))
...
def create_variant(product_id: int):
    ...
    try:
        variant = ProductVariant(
            company_id=product.company_id,
            product_id=product.id,
            name=str(data.get("name", "")).strip()[:180] or None,
            color=str(data.get("color", "")).strip()[:80] or None,
            size=str(data.get("size", "")).strip()[:80] or None,
            sku=str(data.get("sku", "")).strip()[:80] or None,
...
def update_variant(variant_id: int):
    ...
    try:
        for field, maximum in {"name": 180, "color": 80, "size": 80, "sku": 80}.items():
            if field in data:
                setattr(variant, field, str(data[field]).strip()[:maximum] or None)
        if "price" in data:
            variant.price = _money(data["price"], optional=True)
        if "active" in data or "is_active" in data:
            variant.is_active = coerce_bool(data.get("active", data.get("is_active")))
